...
Note: `Effect.catchAll -> Effect.catch` and related `catch*` renames above are
superseded by the more complete `catchFilter`/`catchCauseFilter` picture in
`error-handling.md` — check that file when `catchSome`/`catchSomeCause` are
involved, since those aren't simple renames.